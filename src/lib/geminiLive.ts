...
export interface GeminiLiveOptions {
  token: string; // Ephemeral auth token from server
  systemPrompt: string;
  voice?: string;
      ...
      this.session = await this.ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (e: ErrorEvent) => {
            ...
            this.opts.onError?.(
              new Error((e as ErrorEvent).message || "Live connection error")
            );
            this.setState("error");
          },
          onclose: () => {
        ...
        turns: [
          {
            ...
            parts: [
              {
                text: "[SYSTEM NOTE: time is almost up. Wrap naturally in ONE short sentence, then stop.]",
              },
            ],
          },
        ],
